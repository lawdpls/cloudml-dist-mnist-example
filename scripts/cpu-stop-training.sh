...
for  i in $(seq 0 $NUM_PS); do
  ...
  gcloud compute ssh ps-${i}-cpu -- pkill -f trainer/task.py
done

# Stop workers
for  i in $(seq 0 $NUM_WORKER); do
  echo "Terminating worker-${i}..."
  gcloud compute ssh worker-${i}-cpu -- pkill -f trainer/task.py
done

# Stop a master
echo "Terminating master-0..."
gcloud compute ssh master-0-cpu -- pkill -f trainer/task.py
